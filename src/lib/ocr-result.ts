 ...
 *
 * The free path hands over text, and only text. The sentence for a searchable PDF is
 * describeSearchable in src/pro/searchable.ts. It lived here as a second branch until
 * 12 September 2026 — and a Pro branch in a shared function ships in every production bundle,
 * unreachable, where the Pro flag's sentinel cannot see it. tools/verify-pro-gate.mjs now checks
 * the bundles for its wording.
 */
...
/**
 * Words that assert a file was written. "searchable" is here because it is the exact word that
...
export function describeOcr(o: OcrOutcome): { head: string; announce: string } {
  const { pagesRead, pageCount, fromLayer } = o;
