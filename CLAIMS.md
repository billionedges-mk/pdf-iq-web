...
30. [The only fixture was the one case that worked](#30-the-only-fixture-was-the-one-case-that-worked)
...
### 30. The only fixture was the one case that worked

The site decrypted RC4, AES-128 and AES-256 files. Its only encrypted fixture was an RC4
40-bit file generated by its own `encrypt-fixture.mjs`, which is the one variant the code
handled correctly. AES was written from the specification and never run against a file
anything else had produced.

The first run against the Android app's PdfBox fixtures, done to fix TECH_DEBT 27, failed
**12 of 25 cases**:
...
forbids, and it would have failed the correct fix.
...
back by MuPDF, run against the old code first and watched to fail.
