...
<!-- index: generated by tools/claims-index.mjs — do not edit by hand -->

1. [Numbers the code cannot back](#1-numbers-the-code-cannot-back)  
2. [Predicted output sizes](#2-predicted-output-sizes)  
3. [Timing claims](#3-timing-claims)  
4. [Technology claims](#4-technology-claims)  
5. [Platform-support claims](#5-platform-support-claims)  
6. [Cross-surface claims](#6-cross-surface-claims)  
7. [Price and date claims](#7-price-and-date-claims)  
8. [Stage-cost claims — *added after the merge finding*](#8-stage-cost-claims--added-after-the-merge-finding)  
9. [Relative-speed claims — *added after the OCR finding*](#9-relative-speed-claims--added-after-the-ocr-finding)  
10. [Permanence claims — *added after the account-language sweep*](#10-permanence-claims--added-after-the-account-language-sweep)  
11. [Platform-injected claims — *added after the Cloudflare beacon*](#11-platform-injected-claims--added-after-the-cloudflare-beacon)  
12. [Library error identity — *added after a real encrypted file*](#12-library-error-identity--added-after-a-real-encrypted-file)  
13. [Two ways to satisfy a check means both get tested](#13-two-ways-to-satisfy-a-check-means-both-get-tested)  
14. [An offer must be able to work, and must carry the code that fulfils it](#14-an-offer-must-be-able-to-work-and-must-carry-the-code-that-fulfils-it)  
15. [A false citation is worse than no citation](#15-a-false-citation-is-worse-than-no-citation)  
16. [An instrument needs a check that makes an impossible reading refuse](#16-an-instrument-needs-a-check-that-makes-an-impossible-reading-refuse)  
17. [A silent skip becomes an invented explanation downstream](#17-a-silent-skip-becomes-an-invented-explanation-downstream)  
18. [A status list is a claim, and it goes stale in the direction that flatters](#18-a-status-list-is-a-claim-and-it-goes-stale-in-the-direction-that-flatters)  
19. [A decision on one surface expires claims published on another](#19-a-decision-on-one-surface-expires-claims-published-on-another)  
20. [A redirect is only correct if the destination exists](#20-a-redirect-is-only-correct-if-the-destination-exists)  
21. [A fact about someone else's platform has a shelf life, and you remember the previous version](#21-a-fact-about-someone-elses-platform-has-a-shelf-life-and-you-remember-the-previous-version)  
22. [A design bundle will carry false claims — it is a property, not an incident](#22-a-design-bundle-will-carry-false-claims--it-is-a-property-not-an-incident)  
...
### 27. A check that cries wolf is where a real failure goes to hide

A check that reports a failure it does not act on does not merely get ignored. **It becomes
the safest place in the system for a genuine failure to sit** — the one output everyone has
trained themselves to skip, where a real defect can print itself in full every run and never
be read.

That is worse than having no check at all, because an absent check is at least honestly
absent. This one was accumulating trust it had stopped deserving.

`tools/contrast.mjs` ended with `process.exitCode = 0;` under two FAIL lines it had printed
for months. Every run: two failures, green exit, nobody acts.
...
So the wolf was real, and it had been standing in the noise the whole time. **The false
failures were hiding a true one of exactly the same kind** — same script, same units, same
threshold — and the only reason it went unread is that the two beside it had taught everyone
the section was noise.

The script also hardcoded the four palette hexes rather than reading `app.css`, so it would
have gone on reporting the old palette's ratios after any change to the real one; see
...
### 30. A generator and a consumer that share an author agree with each other

Agreeing is not being correct. Test data written by the same hand as the code that reads it
encodes the same reading of the spec: it exercises the cases that reading got right and cannot
reach the ones it got wrong. That reaches well beyond crypto — any format, protocol or schema
where the fixture and the parser share an author will pass together and fail together, and
only the second half is invisible.

The instance. The site decrypted RC4, AES-128 and AES-256 files. Its only encrypted fixture was
an RC4 40-bit file generated by its own `encrypt-fixture.mjs`, which is the one variant the code
handled correctly. AES was written from the specification and never run against a file
anything else had produced.

The first run against the Android app's crypto fixtures, done to fix TECH_DEBT 27, failed
**12 of 25 cases**:

- **AES-128 output was silently corrupt.** PKCS#7 padding was stripped twice, so a stream or
  string whose last real byte was a newline lost ten bytes. The file opened, pdf-lib accepted
  it, the tool finished, and the page text was gone. Nothing but a second reader could tell.
- **AES-256 did not work at all.** The correct user password threw an uncaught exception on
  one file and was rejected on the other; the correct owner password was rejected on both.
  Three separate defects: `/UE` decrypted with a padding trick that throws on unpadded data,
  `U` passed as salt rather than as the hash's third input, and the Algorithm 2.B loop
  stopping one round early.
- **TECH_DEBT 27**: every restricted file came out unrestricted, including owner-only files
  opened with no password at all.

**And the suite had encoded the last one.** Its end-to-end case unlocked the restricted RC4
fixture with the user password and asserted that the output carried no encryption. When
PASSWORD_RULE.md was decided, the test did not fail — it was asserting the behaviour the rule
forbids, and it would have failed the correct fix. It is the third time on this project a test
encoded the defect it existed to catch. It was split into two tests rather than edited, so each
can fail on its own.

**The checks:**

1. A file-format feature is untested until it has been run against files a different
   implementation wrote. One fixture per variant the code claims to handle, from outside.
2. Verify outputs with a reader that did not write them, on content — the fixture's own
   marker text — not on whether the file opens. "It opened" was satisfied by the corrupt
   output.
3. When a rule is decided, search the tests for assertions of the old behaviour. A test can
   encode the defect, and then it guards it.

`npm run verify:crypto` is the result: 25 cases through the real `openPdf`, each output read
back by MuPDF and pypdf, run against the old code first and watched to fail.

**And a provenance claim nobody checked, inside the entry about exactly that.** For a day both
sessions called these fixtures "PdfBox-made" — in the README, TECH_DEBT, CLAUDE.md, the
verifier's own header and a commit message. Each file's third line reads `% Written by MuPDF
1.27.2`. The label came from where the files were used, not from what wrote them, and nobody
